...
from datasets import CIFAR10_truncated, CIFAR100_truncated, CharacterDataset,CIFAR10_truncated_special_normalize
...
def set_logger(args):
    for handler in logging.root.handlers[:]:
        logging.root.removeHandler(handler) # 移除所有现有的日志处理程序

    args.log_file_name = (
        f"{args.alg}-{args.dataset}-{args.partition}_log-%s"
...
def compute_accuracy_personally(personal_qkv_list, global_model, args, net_dataidx_map_train, net_dataidx_map_test, device="cpu"):
    ...
    for net_id in range(args.n_parties):
        ...
        if args.noise_type == 'space':
            train_dl_local, test_dl_local, _, _ = get_divided_dataloader(args.dataset, args.datadir, args.batch_size,
                                                                         32, dataidxs_train, dataidxs_test, noise_level,
                                                                         net_id, args.n_parties - 1)
        ...
        if args.train_acc_pre:
            train_correct, train_total, train_avg_loss = compute_accuracy_loss(local_model, train_dl_local,
                                                                               device=device)
        logger.info("after aggregation net %d final test acc %f" % (net_id, test_correct / test_total))
        acc[net_id] = test_correct / test_total
    ...
    if args.train_acc_pre:
        ...
        return train_results, train_avg_loss, train_acc_pre, train_all_acc, test_results, test_avg_loss, test_avg_acc, test_all_acc, acc
    else:
        return 0, 0, 0, 0, test_results, test_avg_loss, test_avg_acc, test_all_acc, acc

def compute_accracy_personally_best(best_nets, args, net_dataidx_map_train, net_dataidx_map_test,device="cpu"):
    acc = np.zeros(args.n_parties)
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    test_results = defaultdict(lambda: defaultdict(list))
    for net_id in range(args.n_parties):
        local_model = copy.deepcopy(best_nets[net_id])
        local_model.eval()
        ...
        test_correct, test_total, test_avg_loss = compute_accuracy_loss(local_model, test_dl_local,
                                                                        device=device)
        if args.train_acc_pre:
            train_correct, train_total, train_avg_loss = compute_accuracy_loss(local_model, train_dl_local,
                                                                               device=device)
        logger.info("best selected net %d final test acc %f" % (net_id, test_correct / test_total))
        acc[net_id] = test_correct / test_total
