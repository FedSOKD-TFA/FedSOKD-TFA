...
def get_distribution_difference(client_cls_counts, partipation_clients, metric):
    global_distribution = np.ones(client_cls_counts.shape[1])/client_cls_counts.shape[1]
    local_distributions = client_cls_counts[np.array(partipation_clients),:]
    local_distributions = local_distributions/local_distributions.sum(axis=1)[:,np.newaxis] #归一化得到概率分布

    if metric == 'cosine':
        similarity_scores = local_distributions.dot(global_distribution) / (
                    np.linalg.norm(local_distributions, axis=1) * np.linalg.norm(global_distribution))
